                ...
                if(i < formsData.get_data_limit){
                    ...
                    if(feedData[i].link[0]){
                      ...
                      if(feedData[i].link[0].includes('pff')){
                        try{
                        (async () => {
                           resultMeta = await parse(feedData[i].link[0]);

                                 metaImage = result.images[0].src;
                        ...
                        try{
                          (async () => {
                            var result = await parse(feedData[i].link[0]);

                                   metaImage = result.og.image;
                                  })();
                          }catch(err){
                            console.log(err);
                          }
            ...
            async.forEachOf(feedData, (feed, i, callbackfeedData) => {
                if(i < formsData.get_data_limit){
                    ...
                    let insertPostData = {
                        ...
                        "post_date": postDate,
                        "post_date_gmt": postDate,
                        "post_content": feed.content && feed.content.length ? feed.content[0]._ : "",
                        "post_content_filtered": "",
                ...
                dbConn(getIdsquery, (err, formsData) => {
                    if (err) {
                        ...
                        if (tmpVar.length) {
                            ...
                            // let view_wp_postmeta = "CREATE VIEW view_wp_postmeta AS SELECT post_id, meta_key, meta_value, meta_id  FROM `wp_postmeta` WHERE `meta_key` LIKE 'article_reference_url' order by meta_id desc limit 200; ";
                            // dbConn(view_wp_postmeta, [], async(err, formsData) => { });
                            let Getpostmata = "SELECT  `meta_value` FROM `wp_postmeta` WHERE `meta_key` LIKE 'article_reference_url' ORDER BY meta_id DESC limit 1;"
