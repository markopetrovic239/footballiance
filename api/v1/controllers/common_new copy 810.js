            ...
            CT.load((err, crontab) => {
                ...
                async.forEachOf(crons, (feed, i, callbackfeedData) => {
                    ...
                    if (crons[i].pre_sel_cron_tm) {
                        crontab.create('wget -O /dev/null http://127.0.0.1:3000/v1/auth/setFeed?id=' + crons[i].id, crons[i].pre_sel_cron_tm, 'comment_' + crons[i].id);
                        ...
                        let job = crontab.create('wget -O /dev/null http://127.0.0.1:3000/v1/auth/setFeed?id=' + crons[i].id, 'comment_' + crons[i].id);
                        keyArr.map((key) => {
    ...
    setCronJob: (id, body) => {
        ...
        CT.load((err, crontab) => {
            if (body.pre_sel_cron_tm) {
                crontab.create('wget -O /dev/null http://127.0.0.1:3000/v1/auth/setFeed?id=' + id, body.pre_sel_cron_tm, 'comment_' + id);
                //crontab.create('wget -O /dev/null http://35.237.202.229:3000/v1/auth/setFeed?id=' + id, body.pre_sel_cron_tm, 'comment_' + id); // Old
            } else {
                let keyArr = ["minute", "hour", "month", "dow"];
                //let job = crontab.create('wget -O /dev/null http://35.237.202.229:3000/v1/auth/setFeed?id=' + id, 'comment_' + id); // Old
                let job = crontab.create('wget -O /dev/null http://127.0.0.1:3000/v1/auth/setFeed?id=' + id, 'comment_' + id);
                keyArr.map((key) => {
            ...
            async.forEachOf(feedData, (feed, i, callbackfeedData) => {
                ...
                if (i < formsData.get_data_limit) {
                    ...
                    _self.getMetaData(feedData[i].link[0], (error, metaImage) => {
                        ...
                        if (feedData[i].title && feedData[i].title.length && (feedData[i].title[0].includes(`‘`) || feedData[i].title[0].includes(`’`))) {
                            feedData[i].title[0] = feedData[i].title[0].split("’").join("'");
                            feedData[i].title[0] = feedData[i].title[0].split("‘").join("'");
                        }
            ...
            async.forEachOf(feedData, (feed, i, callbackfeedData) => {
                if (i < formsData.get_data_limit) {
                    _self.getMetaData(feed.id[0], (error, metaImage) => {
                        ...
                        if (feedData.title && feed.title.length && (feedData.title[0].includes(`‘`) || feedData.title[0].includes(`’`))) {
                            feedData.title[0] = feedData.title[0].split("’").join("'");
                            feedData.title[0] = feedData.title[0].split("‘").join("'");
                        }
                        let postDate = momentTZ.tz("US/Eastern").format("YYYY-MM-DD HH:mm:ss");
                        let insertPostData = {
    ...
    getFeedNewAdmin: (req, res) => {
        ...
        async.waterfall([
            ...
            if (err) {
                ...
                try {
                    dbConn(query, [datetime + " Failed", PostFormId], (err, insertSucc) => {
            ...
            try {
                ...
                dbConn(query, [datetime + " Success", PostFormId], (err, insertSucc) => {
