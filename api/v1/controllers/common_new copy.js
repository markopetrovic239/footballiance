        ...
        if (result && result.rss && result.rss.channel.length && result.rss.channel[0].item.length) {
            ...
            async.forEachOf(feedData, (feed, i, callbackfeedData) => {
                ...
                if(i < formsData.get_data_limit){
                  ...
                  // Ensure that links match the camp name website
                  let link = 'url';
                  if(feedData[i].link[0]){
                      link = feedData[i].link[0]
                      console.log(link);
                  }
                 if (feedData[i].link[0] && JSON.stringify(link).includes(formsData.camp_name.toLowerCase())){
                    let postDate = momentTZ.tz("US/Eastern").format("YYYY-MM-DD HH:mm:ss");
                    let insertPostData = {
                        "post_author": formsData.author,
                        "post_date": postDate,
                    ...
                    if(!PostMetaArray.includes(feedData[i].link[0])){
            ...
            async.forEachOf(feedData, (feed, i, callbackfeedData) => {
                if(i < formsData.get_data_limit){
                 ...
                 if (feedData[i].link[0] && JSON.stringify(link).includes(formsData.camp_name.toLowerCase())){
                    ...
                    let postImage = myRegex.exec(feed.content[0]._);
                    ...
                    let insertPostData = {
                        ...
                        "post_date": postDate,
                        "post_date_gmt": postDate,
                        "post_content": feed.content.length ? feed.content[0]._ : "",
                        "post_content_filtered": "",
                        "post_title": feed.title.length ? feed.title[0] : "",
                        "post_excerpt": "",
                        "post_status": "publish",
                        "post_type": "post",
                        "comment_status": "open",
                    ...
                    let insertPostDataquery = "insert into `wp_posts` SET ?";
                    //feed.id[0] feed ID array for link positions
                ...
                dbConn(getIdsquery, (err, formsData) => {
                    if (err) {
                        ...
                        if (tmpVar.length) {
                            ...
                            // let view_wp_postmeta = "CREATE VIEW view_wp_postmeta AS SELECT post_id, meta_key, meta_value, meta_id  FROM `wp_postmeta` WHERE `meta_key` LIKE 'article_reference_url' order by meta_id desc limit 200; ";
                            // dbConn(view_wp_postmeta, [], async(err, formsData) => { });
                            let Getpostmata = "SELECT  `meta_value` FROM `wp_postmeta` WHERE `meta_key` LIKE 'article_reference_url' ORDER BY meta_id DESC limit 2000;"
