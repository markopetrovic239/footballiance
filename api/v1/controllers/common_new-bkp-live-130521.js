...
var _self = {
            ...
            CT.load((err, crontab) => {
                ...
                async.forEachOf(crons, (feed, i, callbackfeedData) => {
                    ...
                    if (crons[i].pre_sel_cron_tm) {
                        crontab.create('wget -O /dev/null http://34.73.154.2:3000/v1/auth/setFeed?id=' + crons[i].id, crons[i].pre_sel_cron_tm, 'comment_' + crons[i].id);
                        ...
                        let job = crontab.create('wget -O /dev/null http://34.73.154.2:3000/v1/auth/setFeed?id=' + crons[i].id, 'comment_' + crons[i].id);
                        keyArr.map((key) => {
    ...
    addFeedDataIntoDB: async (result, formsData, addFeedDataCallback) => {
        if (result && result.rss && result.rss.channel && result.rss.channel.length && result.rss.channel[0].item && result.rss.channel[0].item.length) {
            let feedData = result.rss.channel[0].item;
            console.log("addFeedDataIntoDB Start =======================================>", formsData.camp_name)
